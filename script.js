...
function setupEventListeners() {
    ...
    const keepMysteryBtn = document.getElementById('keepMysteryBtn');
    if (keepMysteryBtn) {
        keepMysteryBtn.addEventListener('click', () => {
            showToast('info', 'Mystery Kept', 'Your destination will remain a surprise until the reveal date!');
            navigateTo('trips');
        });
    }
...
function viewTrip(tripId) {
    ...
    // Enable trip action buttons
    document.getElementById('editTripButton').onclick = () => editTrip(tripId);
    document.getElementById('deleteTripButton').onclick = () => deleteTrip(tripId);
    document.getElementById('revealButton').onclick = () => revealDestination(tripId);
    document.getElementById('keepMysteryBtn').onclick = () => keepMystery(tripId);
...
function revealDestination(tripId) {
    if (!tripId) {
        tripId = document.getElementById('tripResultPage').dataset.tripId;
    }
    const trip = getTrip(tripId);
    if (!trip) return;

    // Immediately reveal destination and mark as revealed
    trip.revealed = true;
    updateTrip(trip);
    ...
    // Hide reveal section and show destination info
    document.getElementById('revealSection').style.display = 'none';
    document.getElementById('destinationInfo').style.display = 'block';

    // Update all destination information immediately
    document.getElementById('resultImage').src = destination.image;
    document.getElementById('resultTitle').textContent = destination.name;
    document.getElementById('destinationDescription').textContent = destination.description;
    document.getElementById('visaInfo').textContent = destination.visa;
    document.getElementById('flightInfo').textContent = destination.flights;
    document.getElementById('hotelInfo').textContent = destination.hotels;
    document.getElementById('budgetInfo').textContent = destination.budget;
    document.getElementById('tipsInfo').textContent = destination.tips;
    document.getElementById('emergencyInfo').textContent = destination.emergency;
    document.getElementById('bestTimeInfo').textContent = destination.bestTime;
    ...
    // Show notification
    showNotification('Destination Revealed!', `Your destination is ${destination.name}! Click to view details.`, {
        tag: `trip-reveal-${trip.id}`,
        data: { tripId: trip.id }
    });

    showToast('success', 'Destination Revealed!', `Welcome to ${destination.name}!`);
}

function keepMystery(tripId) {
    if (!tripId) {
        tripId = document.getElementById('tripResultPage').dataset.tripId;
    }
    const trip = getTrip(tripId);
    ...
    // Set reveal date to 24 hours from now
    ...
    // Create mystery trip card
    const mysteryTripCard = document.createElement('div');
    mysteryTripCard.className = 'mystery-trip-card';
    mysteryTripCard.innerHTML = `
        <div class="mystery-header">
            <h3>Mystery Trip</h3>
            <div class="countdown" id="countdown-${trip.id}">Loading countdown...</div>
        </div>
        <div class="trip-preferences">
            ...
            <div class="preference-item">
                ...
                <span>${trip.travelStyles.join(', ')}</span>
            </div>
        </div>
    ...
    const tripsList = document.getElementById('tripsList');
    if (tripsList) {
        tripsList.insertBefore(mysteryTripCard, tripsList.firstChild);
    }
    ...
    // Start countdown
    startCountdown(trip.id);

    // Show confirmation and redirect
    showToast('success', 'Mystery Trip Created', 'Your destination will be revealed when the countdown ends!');
    navigateTo('trips');
}
